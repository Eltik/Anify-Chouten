...
export class MyCloudE extends VideoExtractor {
    ...
    override extract = async (): Promise<ISource> => {
        ...
        const mediaInfoResult = await request("https://anithunder.vercel.app/api/mcloud", "POST", { "Content-Type": "application/json" }, JSON.stringify({ url: url }));

        const mediaInfo: IMediaInfo = JSON.parse(mediaInfoResult.body);
