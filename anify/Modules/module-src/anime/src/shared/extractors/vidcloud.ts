...
export class VidCloudE extends RawVideoExtractor {
    ...
    override extract = async (): Promise<ISource> => {
        ...
        const data: ResponseType = await request("https://rabbitthunder-test.vercel.app/api/vidcloud", "GET", { "Content-Type": "application/json" }, JSON.stringify({ id: id })).then((req) => req.json());
