...
export class NetuE extends VideoExtractor {
    ...
    override extract = async (): Promise<ISource> => {
        ...
        const v = iframe_url.replace("/e/", "").split("?")[0];
        iframe_url = hostname + iframe_url;

        const iframe_text = await request(iframe_url, "GET").then((resp) => resp.body);
        ...
        const image_res: PlayerImage | PlayerImageFail = await request(
            hostname + "/player/get_player_image.php",
            "POST",
            {
        ...
        const playButtonCoords: number[] = await request("https://mochi_back.nixuge.me/epicImageFinder", "POST", { "Content-Type": "application/json" }, JSON.stringify({ b64image: image_res.image })).then((req) => req.json());
        ...
        const finalResp: DataResponse = await request(hostname + "/player/get_md5.php", "POST", headers, JSON.stringify(body)).then((resp) => resp.json());

        const link = finalResp.obf_link;
        if (!link || link == "#") {
            if (finalResp.ip) finalResp.ip = "xxx.xxx.xxx.xxx"; // Avoid leaking ips when sending logs
            throw Error("Failed to grab link!" + JSON.stringify(finalResp));
        }

        let clearLink = deobfLink(link);
        if (clearLink.startsWith("//")) clearLink = "https:" + clearLink;
        else if (clearLink.startsWith("/")) clearLink = hostname + clearLink;
