...
var source = (() => {
  ...
  // src/aniwave/aniwave.ts
  var aniwave_exports = {};
  __export(aniwave_exports, {
    default: () => AniwaveModule
  });
    ...
    for (let i = 0; i < numElements; i++) {
      ...
      if (!className) {
        ...
        for (let j = 0; j < classNames.length; j++) {
          const appendClass = "".concat(classNames[j], " ");
  ...
  // src/aniwave/scraper/homeScraper.ts
  var HomeScraper = class {
    constructor(baseName) {
      this.baseName = baseName;
    }
    async scrape() {
      const html3 = await request("".concat(this.baseName, "/home"), "GET");
      if (html3.statusCode != 200) {
        let cookies = await callWebview("".concat(this.baseName, "/home"));
        console.log(JSON.stringify(cookies));
        const html22 = await request("".concat(this.baseName, "/home"), "GET");
        this.$ = load(html22.body);
    ...
    scrapeHotest() {
      const $2 = this.$;
      const swiper = $2(".swiper-wrapper .swiper-slide.item");
      const hotestItems = swiper.map((_i, anime) => {
        var _a2;
        const animeRef = $2(anime);
        const title = animeRef.find("h2.title.d-title").text();
        let image = (_a2 = animeRef.find("div.image > div").attr("style")) == null ? void 0 : _a2.replace("background-image: url('", "").replace("');", "");
        if (image == void 0) {
          image = null;
        }
    ...
    scrapeRecentlyUpdated() {
      const $2 = this.$;
      const recentUpdatesSec = $2(".ani.items .item");
      const recentlyUpdatedItems = recentUpdatesSec.map((_i, anime) => {
        ...
        let image = animeRef.find("div.ani.poster > a > img").attr("src");
        if (image == void 0) {
          image = null;
        }
        return {
    ...
    scrapeBottomThreeColumns(id) {
      const table = this.$("div.top-tables.mb-3 .body .top-table").filter(
        (_i, _table) => {
          let table2 = _table;
          return table2.attribs["data-name"] == id;
        }
      )[0];
      const $2 = load(table);
      const title = $2("div.head a.title").contents().first().text();
      const items = $2("div.body div.scaff.items a.item").map((_i, item) => {
        var _a2;
        const itemRef = $2(item);
        const url = itemRef.attr("href");
        let image = (_a2 = itemRef.find("div.poster span img").attr("src")) == null ? void 0 : _a2.replace("-w100", "-w9999");
        if (image == void 0) {
          image = null;
        }
        const title2 = itemRef.find("div.info div.name.d-title").text();
        return {
          url,
          titles: {
            primary: title2
  ...
  // src/aniwave/utils/variables.ts
  var AJAX_BASENAME = "https://aniwave.to/ajax";

  // src/aniwave/utils/urlGrabber.ts
  function b64encode(data2) {
    console.log(data2);
    data2 = "".concat(data2);
    for (s = 0; s < data2.length; s++) {
      if (255 < data2.charCodeAt(s)) {
        return null;
      }
    }
    var r = "";
    for (var s = 0; s < data2.length; s += 3) {
      var o = [void 0, void 0, void 0, void 0];
      o[0] = data2.charCodeAt(s) >> 2;
      o[1] = (3 & data2.charCodeAt(s)) << 4;
      if (data2.length > s + 1) {
        o[1] |= data2.charCodeAt(s + 1) >> 4;
        o[2] = (15 & data2.charCodeAt(s + 1)) << 2;
      }
      if (data2.length > s + 2) {
        o[2] |= data2.charCodeAt(s + 2) >> 6;
        o[3] = 63 & data2.charCodeAt(s + 2);
      }
  ...
  function rc4Cypher(key, data2) {
    ...
    for (var h = 0; h < data2.length; h++) {
      ...
      r[u] = r[s = (s + r[u]) % 256];
      r[s] = n;
      o += String.fromCharCode(data2.charCodeAt(h) ^ r[(r[u] + r[s]) % 256]);
    }
    return o;
  }
  function idToVrf(t) {
    ...
    return function(t2) {
      var s = 8;
      t2 = serializeText(caesarRot13(caesarRot13(t2)));
      var r = "";
      for (var o = 0; o < t2.length; o++) {
        var h = t2.charCodeAt(o);
        if (o % s == 7) {
          h += 6;
        } else if (o % s == 5) {
          h -= 3;
  ...
  function caesarRot13(t) {
    return t.replace(/[a-zA-Z]/g, function(t2) {
      const res = String.fromCharCode(
        (t2 <= "Z" ? 90 : 122) >= (t2 = t2.charCodeAt(0) + 13) ? t2 : t2 - 26
      );
      return res;
    });
  }
  function b64decode(t) {
    ...
    for (var h = 0; h < t.length; h++) {
      r = t[h];
      o = (o <<= 6) | ((r = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".indexOf(
        r
      )) < 0 ? void 0 : r);
      if (24 === (u += 6)) {
  ...
  function decodeVideoSkipData(encoded_url) {
    console.log(encoded_url);
    encoded_url = b64decode(
      "".concat(encoded_url).replace(/_/g, "/").replace(/-/g, "+")
    );
    const decoded_url = decodeURIComponent(
      rc4Cypher("LUyDrL4qIxtIxOGs", encoded_url)
    );
    return decoded_url;
  }
  function getVrf(input) {
    return encodeURIComponent(idToVrf(input));
  }

  // src/aniwave/scraper/mediaList.ts
  async function grabMediaList(baseName, _url) {
    const watch = _url.startsWith("/watch/") ? "" : "/watch/";
    const fullUrl = "".concat(baseName).concat(watch).concat(_url);
    const html3 = await request(fullUrl, "GET");
    if (html3.body.includes("<title>WAF</title>")) {
      console.error(
        "Seems like you're getting captcha'd. Unfortunately I can't do much about it. Retry later/change your ip."
      );
      return [];
    }
    const $2 = load(html3.body);
    const data_id = $2("div#watch-main").attr("data-id");
    const url = "".concat(AJAX_BASENAME, "/episode/list/").concat(data_id, "?vrf=").concat(getVrf(data_id != null ? data_id : ""));
    console.log("url: " + url);
    let episodesHtml;
    try {
      episodesHtml = JSON.parse(
        (await request(url, "GET", { "x-requested-with": "XMLHttpRequest" })).body
      )["result"];
    } catch (e) {
    ...
    epRange.map((_, li) => {
      ...
      const episodeTitle = (_a2 = titleDiv.text()) != null ? _a2 : void 0;
      let variantReleaseDates = inScraper.attr("title");
      variantReleaseDates = variantReleaseDates == null ? void 0 : variantReleaseDates.matchAll(
      ...
      for (let match of variantReleaseDates) {
        ...
        playlistGroup == null ? void 0 : playlistGroup.pagination[0].items.push({
          url: dataIds + " | " + variantType.toLowerCase(),
          // kinda dirty but idk how else to pass data through to the next step,
          number: episodeNum,
          title: episodeTitle
          // TODO: See if image & description are available somewhere
        });
        console.log("".concat(playlistGroup));
      }
    });
    console.log("".concat(answer[0]));
    return answer;
  }

  // src/aniwave/utils/skipData.ts
  function parseSkipData(data2) {
    if (data2) {
  ...
  // src/shared/models/Iextractor.ts
  function isVideoArray(obj) {
    return Array.isArray(obj);
  }
  var RawVideoExtractor = class {
    constructor(referer) {
  ...
  var VideoExtractor = class extends RawVideoExtractor {
    constructor(referer, htmlContent) {
      super(referer);
  ...
  // src/shared/utils/m3u8.ts
  var M3U8_PATTERN = /#EXT-X-STREAM-INF:[^\n]*RESOLUTION=\d+x(\d+)[^\n]*\n([^\n]+)/g;
  async function getM3u8Qualities(mainFileUrl, opts = {}) {
    ...
    const sources = [];
    if (opts.keepAuto === void 0 || opts.keepAuto)
      sources.push({
        url: mainFileUrl,
        quality: "auto",
        isDASH: false
    ...
    let m3u8Data = "";
    try {
      let data2 = await request(mainFileUrl, "GET", opts.headers);
      if (data2.statusCode != 200) {
        await callWebview(mainFileUrl);
        data2 = await request(mainFileUrl, "GET", opts.headers);
      }
      m3u8Data = data2.body;
    } catch (e) {
  ...
  // src/shared/extractors/filemoon.ts
  function packed(p, a, c, k) {
    while (c--) if (k[c]) p = p.replace(new RegExp("\\b" + c.toString(a) + "\\b", "g"), k[c]);
    return p;
  }
  var FilemoonE = class extends VideoExtractor {
    constructor() {
      super(...arguments);
      this.serverName = "filemoon";
      this.extract = async () => {
        var _a2;
        const html3 = this.htmlContent;
        const match = (_a2 = html3.match(/return p}\('(.*?\))\)\)/)) == null ? void 0 : _a2[1];
        ...
        return {
          videos,
          headers: { "User-Agent": "Chrome trust not a phone I promise" }
          // "Android" and "iPhone" are flagging lol
        };
      };
    }
    dirtyArrayExtract(rawData) {
  ...
  // src/shared/extractors/mp4upload.ts
  var Mp4uploadE = class extends VideoExtractor {
    constructor() {
      super(...arguments);
      ...
      this.extract = async () => {
        var _a2;
        const html3 = this.htmlContent;
        const url = (_a2 = html3.match(/rplayer\.src.|\n*?src: "(.*?)"/)) == null ? void 0 : _a2[1];
        return {
          videos: [{
            url,
            quality: "auto",
  ...
  // src/shared/utils/subtitles.ts
  function isSubtitle(object) {
    return "kind" in object && object["kind"] == "captions";
  }
  function parseSubtitles(tracks, type = 0 /* VTT */) {
    const subtitles = [];
  ...
  // src/shared/extractors/mycloud.ts
  var MyCloudE = class extends VideoExtractor {
    constructor() {
      super(...arguments);
      this.serverName = "mycloud";
      this.extract = async () => {
        const url = this.referer;
        const mediaInfoResult = await request(
          "https://anithunder.vercel.app/api/mcloud",
          "POST",
          { "Content-Type": "application/json" },
          JSON.stringify({ url })
        );
        const mediaInfo = JSON.parse(mediaInfoResult.body);
  ...
  var AniwaveModule = class extends BaseModule {
    ...
    async search(query, page) {
      ...
      const resp = await request(
        "".concat(this.baseName, "/filter?keyword=").concat(encodeURIComponent(query), "&page=").concat(page),
        "GET"
      );
      const $2 = load(resp.body);
      const items = $2("#list-items > div.item").map((_i, anime) => {
        var _a3, _b, _c;
        const animeRef = $2(anime);
        const metaRef = animeRef.find("div.b1 > a.name.d-title");
        const url = (_b = (_a3 = metaRef.attr("href")) == null ? void 0 : _a3.split("/").pop()) != null ? _b : "";
        const name = metaRef.text();
        const img = (_c = animeRef.find("div > a > img").attr("src")) != null ? _c : "";
        return {
          url: "/watch/".concat(url),
          title: name,
          poster: img,
          indicator: ""
        };
      }).get();
      const totalPages = (_a2 = $2(".pagination > li").last().attr("href")) == null ? void 0 : _a2.split("page=")[1];
      console.log(totalPages);
      return {
        info: {
          pages: parseInt(totalPages != null ? totalPages : "1")
        },
        results: items
      };
    }
    async info(_url) {
      var _a2;
      const watch = _url.startsWith("/watch/") ? "" : "/watch/";
      const fullUrl = "".concat(this.baseName).concat(watch).concat(_url);
      const html3 = await request(fullUrl, "GET");
      const $2 = load(html3.body);
      const synopsis = $2(".info .synopsis .shorting .content").text();
      const mainTitle = $2(".info h1.title.d-title").text();
      const altTitles = $2(".info .names.font-italic").text().split("; ").filter((altTitle2) => altTitle2 != mainTitle);
      let altTitle = altTitles.length == 0 ? void 0 : altTitles[0];
      let altPoster = (_a2 = $2("div#player").attr("style")) == null ? void 0 : _a2.replace("background-image:url('", "").replace("')", "");
      const seasonSlides = $2(".seasons.swiper-wrapper .swiper-slide");
      const seasons = [];
      if (seasonSlides.length > 0) {
        seasonSlides.map((_i, season) => {
          const seasonRef = $2(season);
    ...
    async servers(_url) {
      ...
      const html3 = await request(
        "".concat(AJAX_BASENAME, "/server/list/").concat(episodeId, "?vrf=").concat(getVrf(episodeId)),
        "GET"
      );
      const json = JSON.parse(html3.body);
      const $2 = load(json["result"]);
      const servers = $2(".type").map((_, serverCategory) => {
        const categoryRef = $2(serverCategory);
        const sourceType = categoryRef.attr("data-type");
        if (sourceType != variantType) return void 0;
        return categoryRef.find("ul").find("li").map((_2, server) => {
          const serverRef = $2(server);
          const serverName = serverRef.text();
          const linkId = serverRef.attr("data-link-id");
          return {
            name: "".concat(serverName),
            url: linkId
          };
        }).get();
      }).get();
    ...
    async sources(_url) {
      ...
      try {
        let data2 = await request(
          "".concat(AJAX_BASENAME, "/server/").concat(_url, "?vrf=").concat(getVrf(_url)),
          "GET"
        );
        if (data2.statusCode != 200) {
          await callWebview("".concat(AJAX_BASENAME, "/server/").concat(_url, "?vrf=").concat(getVrf(_url)));
          data2 = await request(
            "".concat(AJAX_BASENAME, "/server/").concat(_url, "?vrf=").concat(getVrf(_url)),
            "GET"
          );
        }
        const result = JSON.parse(data2.body)["result"];
        ...
        const mediaSource = {
          sources: videos.map((video) => {
            var _a3;
            return {
              quality: (_a3 = video.quality) != null ? _a3 : "auto",
              file: video.url,
              type: 0 /* HLS */
            };
          }),
          skips: skipData,
          subtitles: (_a2 = sourceData.subtitles) != null ? _a2 : [],
          previews: []
          // Pretty sure Aniwave doesn't send those.
        };
        console.log("".concat(mediaSource));
        return mediaSource;
      } catch (error) {
  ...
  return __toCommonJS(aniwave_exports);
})();
